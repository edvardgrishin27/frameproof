...
def test_прореживание_бережёт_яркие_переходы_а_не_плотные():
    """Под потолком кадров первой уходит мелочь, а не заметная смена экрана.

    Регрессия из отзыва: экранные врезки пропадали из индекса при настройках по
    умолчанию. Избыточность считалась по одному лишь получившемуся разрыву, а
    содержательность добавляла 0.001 — то есть решала только точную ничью. Значит
    «дешевле всего» было выкинуть кадр, у которого соседи стоят теснее прочих, — и
    заметная врезка в плотном месте уходила раньше вялого сдвига в разреженном.
    Измеренный выигрыш правки на живом ролике — плюс одна врезка из восьми при том же
    числе кадров (первая присланная оценка «четыре из девяти» её автором отозвана как
    сделанная на глаз).

    Сетку прореживание почти никогда не трогает не по доброте, а по устройству:
    страховочные точки стоят ровно по гарантии, и удаление любой сразу рвёт её.
    Поэтому реально выбор идёт МЕЖДУ переходами, и решать его обязана сила
    изменения, а не плотность соседей.
    """
    events = [float(t) for t in range(30, 580, 6)]
    inserts = set(events[::11])                 # заметные врезки среди мелкой возни
    sig = make_signal(600.0, events=events, magnitude=0.05)
    for t in inserts:
        sig.cells[int(round(t * sig.fps)), 0, 1] = 0.60

    # Экран стоит и переключается только на событиях. Заготовка из make_signal
    # шевелит каждый пиксель каждый кадр — на таком материале вся картинка попадает
    # в маску шума, и дедуп схлопывает соседние переходы в один.
    sig.thumbs[:] = 0
    for n, t in enumerate(events):
        sig.thumbs[int(t * sig.fps):] = 0 if n % 2 else 180   # шаг выше PIXEL_DELTA

    full = select_frames(sig, 600.0, max_gap=15.0, cap=0)
    strong = {round(p.t, 1) for p in full.picks if p.reason == "change" and p.change > 0.5}
    assert len(strong) == len(inserts), "детектор не поймал врезки — тест проверяет не то"

    thinned = select_frames(sig, 600.0, max_gap=15.0, cap=len(full.picks) // 2)
    assert thinned.dropped_budget > 0, "прореживание не сработало — тест вырожден"
    survived = {round(p.t, 1) for p in thinned.picks} & strong
    assert survived == strong, (
        f"прореживание съело врезки: было {len(strong)}, осталось {len(survived)}"
    )
    assert thinned.actual_max_gap <= 15.0 + 1e-6, "прореживание порвало гарантию"
