...
def _thin(picks: list[Pick], cap: int, duration: float, max_gap: float) -> tuple[list[Pick], int]:
    ...
    Здесь на каждом шаге удаляется самый ИЗБЫТОЧНЫЙ кадр. Кадр, чьё удаление порвало
    бы гарантию, не трогается вообще. Если под потолок так не ужаться — оставляем как
    есть, и отчёт честно покажет перебор, вместо того чтобы молча ослепнуть.

    Избыточность считается по СОДЕРЖАНИЮ, и только при равном содержании — по времени.
    Сначала уходят все точки сетки, потом якоря по речи, и лишь потом пойманные
    переходы, слабые раньше сильных.

    Раньше здесь сравнивался один получившийся разрыв, а содержательность работала
    добавкой в 0.001. Разрывы меряются в секундах, поэтому добавка решала только
    точную ничью, а на практике порядок был обратный задуманному: пойманный переход,
    у которого соседи близко по времени, всегда оказывался дешевле точки сетки,
    стоящей в разреженном месте. Экранная врезка на 5-8 секунд — ровно такой кадр:
    содержательно единственный, по времени тесный, и уходил он первым.

    Насколько это стоило в кадрах. Первая присланная оценка — «из девяти врезок
    доходили четыре» — её автором отозвана: список он составил на глаз, а не измерил.
    По сверхплотному проходу с разрывом 2 с на том ролике восемь настоящих врезок.
    Измеренный эффект правки скромнее заявленного: ПЛЮС ОДНА врезка при том же числе
    кадров, одинаково при гарантии 15 и 5. Причина при этом подтверждена по коду, а не
    по замеру, — старая формула стоимости прямо предпочитала удалять плотное.

    Обойти это уменьшением --max-gap можно, но лечит оно не сетку: при пятисекундной
    гарантии почти любое удаление рвёт покрытие, и прореживание просто перестаёт
    работать — ценой примерно тройного числа кадров.
    """
    if len(picks) <= cap:
        return picks, 0
    ...
    dropped = 0
    while len(kept) > cap:
        best_i, best_cost = None, (99, float("inf"), float("inf"))
        for i in range(len(kept)):
            ...
            # Ступень содержательности решает первой, разрыв — только внутри ступени.
            # Точка сетки не несёт ничего своего, якорь по речи держит момент, на
            # который человек показал голосом, пойманный переход — единственный кадр
            # своего экрана. Внутри переходов слабый уходит раньше сильного.
            tier = 0 if kept[i].is_safety else (1 if kept[i].is_cue else 2)
            strength = 0.0 if tier < 2 else kept[i].change
            cost = (tier, strength, new_gap)
            if cost < best_cost:
